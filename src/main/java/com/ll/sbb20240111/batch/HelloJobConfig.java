...
public class HelloJobConfig {
    ...
    @Bean
    public Job byeJob(JobRepository jobRepository, Step byeStep1) {
        return new JobBuilder("byeJob", jobRepository)
                .incrementer(new RunIdIncrementer())
                .start(byeStep1)
                .incrementer(new RunIdIncrementer())
                .build();
    ...
    @Bean
    public Step byeStep1(JobRepository jobRepository, Tasklet byeStep1Tasklet1, PlatformTransactionManager platformTransactionManager) {
        return new StepBuilder("byeStep1Tasklet1", jobRepository)
                .tasklet(byeStep1Tasklet1, platformTransactionManager)
                .build();
    }
    @Bean
    public Tasklet byeStep1Tasklet1() {
        return ((contribution, chunkContext) -> {
            log.info("Bye World");
